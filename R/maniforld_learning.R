...
setwd('~/Downloads/diffusion_maps/')
...
#if we want to test the pangenome

m=t(pan[,grepl('Marinobacter|Tamilna',colnames(pan))])
...
ggplot(dat, aes(x = dim_2, y = dim_3, fill = dim_1)) +
  theme_classic() +
  xlab('Variable 1') +
  ylab('Variable 2') +
  geom_hline(yintercept = 0, size = 0.25, colour = '#bdbdbd') +
...
ggplot(dat, aes(x = dim_6, y = dim_7, fill = dim_1)) +
  theme_classic() +
  xlab('Variable 1') +
  ylab('Variable 2') +
  geom_hline(yintercept = 0, size = 0.25, colour = '#bdbdbd') +
